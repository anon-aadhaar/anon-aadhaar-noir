...
const main = () => {
    ...
    const pkData = readFileSync(path.join(__dirname, '../assets', certificateName));
    ...
    const pubKey = BigInt('0x' +
        bufferToHex(Buffer.from(pk.export({ format: 'jwk' }).n, 'base64url')));
    ...
    writeFileSync(path.join(__dirname, toml_path), TOML.stringify(input));
    console.log('Generated file at', toml_path);
};
main();
//# sourceMappingURL=generateNoirToml.js.map